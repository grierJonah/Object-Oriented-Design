...
/**
 * An interface for the UserPassPair.
 */
public interface IUpp {

  /**
   * The User Pass Pair is a unique 'username, password' string combination that helps authenticate
   * and manage users, username's and password's.
   * The NonEmptyUpp takes two arguments:
   *  - String userName,
   *  - String password
   * A NonEmptyUpp can be two things:
   *  - Non empty (u,p)
   *  - Empty
   */
   ...
   */
  IUpp add(String userName, String passWord) throws PairAlreadyInitializedException;
   ...
   */
  void replace(String userName, String passWord) throws PairEmptyException;
   ...
   * UserPassPair is empty then the method will return an empty UserPassPair. If the
   ...
   */
  IUpp delete(String userName) throws InvalidUsernameException;
   ...
   * then an InvalidUsernameException will be thrown. If the UserPassPair is non-empty (u,p) and
   * the user matches the userName on file, the user will get their password, represented as a
   * String. Lastly, if the UserPassPair is non-empty (u, p) but the user fails to match the
   * userName on file, then an InvalidUsernameException is thrown.
   *
   * @param userName the selected userName, represented as a String
   ...
   */
  String getPassword(String userName) throws InvalidUsernameException;
}
